...
private func semanticHelperDescription(in line: String) -> String? {
    ...
    let helperDescriptions: [(String, String)] = [
        ...
        ("hide_text_cursor(", "Uses BIOS cursor-shape programming to hide the text-mode cursor."),
        ("bios_set_cursor_position(", "BIOS INT 10h AH=02h: moves the text cursor to the given page, row, and column."),
        ("bios_clear_text_window(", "BIOS INT 10h AH=06h with AL=0: clears a text window using the supplied attribute and rectangle."),
